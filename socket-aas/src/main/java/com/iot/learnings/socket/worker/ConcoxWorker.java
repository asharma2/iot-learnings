...
import com.iot.learnings.event.ConcoxEvent;
import com.iot.learnings.event.Event;
import com.iot.learnings.socket.exception.WorkerClosedException;

public class ConcoxWorker extends IotWorker {

	private static final int CRC_TAB6[] = { 0X0000, 0X1189, 0X2312, 0X329B, 0X4624, 0X57AD, 0X6536, 0X74BF, 0X8C48,
	...
	public ConcoxWorker(String connectionID, Socket socket, KafkaTemplate<String, Event> iotEventKafkaTemplate,
			int delayBetweenPacketsInMillis) {
		super(connectionID, socket, iotEventKafkaTemplate, delayBetweenPacketsInMillis);
	}

	@Override
	protected boolean isPacketCompete(List<Integer> packets) {
		int size = packets.size();
		if (size > 4) {
			return packets.get(0) == 120 && packets.get(1) == 120 && packets.get(size - 2) == 13
					&& packets.get(size - 1) == 10;
		}
		return false;
	}

	protected int crc16(List<Integer> packets) {
		int fcs = 0xffff; //
		for (int q = 2; q <= 5; q++) {
			fcs = (fcs >> 8) ^ CRC_TAB6[(fcs ^ packets.get(q)) & 0xff];
		}
		return fcs = fcs ^ 0Xffff;
	}

	@Override
	protected void processPacket(List<Integer> packets, Socket socket) {
		List<Integer> response = new ArrayList<Integer>(packets);
		...
		if (doAcknowledge) {
			int errorCode = crc16(response);
			int e1 = (errorCode >> 8) & 0x00000ff;
			int e2 = errorCode & 0x00000ff;
			response.add(6, e1);
			response.remove(7);
			response.add(7, e2);
			response.remove(8);
			try {
				for (Integer p : packets) {
					socket.getOutputStream().write(p);
				}
				socket.getOutputStream().flush();
			} catch (Exception e) {
				throw new WorkerClosedException(e);
			}
		}
		iotEventKafkaTemplate.send("socket.aas.event.concox", getConnectionID(),
				new ConcoxEvent(getConnectionID(), LocalDateTime.now(ZoneOffset.UTC), packets));
	}
